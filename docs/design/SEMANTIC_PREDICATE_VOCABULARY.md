...
| VF-04 | A relation the theory needs is inexpressible in the format | **MEASURED, THEN CLOSED** `hierarchy_edges` carried one `edge_type` across 1,774 edges. Closed 2026-09-08: the pack now ships `predicate_relations`, `role_correspondences` and `symmetric_role_pairs` as first-class sections in `0.3.3`, and onto-canon6 loads and validates them (PR #375) | Inverse pairs (buy/sell, lend/borrow) could not be declared, so converse phrasings stayed unrelated | Enumerate the relation kinds the design commits to, then grep the schema for each | Done as explicit sections rather than a new `edge_type` — overloading `hierarchy_edges` would have left the rows referentially unchecked, since its validation only inspects `subtype_of` rows |
...
| | pairs | consequence |
|---|---|---|
| `same-object` whose two sides expect **different** predicates | **15 of 24** | Was unreachable: nothing related `lc:acquire_get_obtain` to `lc:buy_purchase`. **Closed 2026-09-08** — 16 predicate relations ship in `0.3.3` and `check_collapse_reachability.py` reports `same_UNREACHABLE 0` (VF-04). Reachable is not the same as correctly collapsed; two of the sixteen were found false and corrected before release. |
